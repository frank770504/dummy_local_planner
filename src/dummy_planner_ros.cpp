...
namespace dummy_local_planner {
  ...
  bool DummyPlannerROS::dummyComputeVelocityCommands(tf::Stamped<tf::Pose> &global_pose, geometry_msgs::Twist& cmd_vel) {
    ...
    */
    ...
    if(path.cost_ < 0) {
      ROS_DEBUG_NAMED("dummy_local_planner",
          "The dummy local planner failed to find a valid plan, cost functions discarded all candidates. This can mean there is an obstacle too close to the robot.");
      local_plan.clear();
